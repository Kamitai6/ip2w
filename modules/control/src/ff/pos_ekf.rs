...
impl PositionEkf {
    pub fn new(cfg: PosEkfConfig) -> Self {
        ...
        Self {
            ...
            prev_a_raw: 0.0, a_tangential_lp: 0.0,
    ...
    pub fn update(&mut self, ax_g: f32, pitch_sensor: f32, pitch_cg: f32, angular_accel: f32) -> f32 {
        let dt = self.cfg.dt;
        ...
        // ───── 3. 状態予測（減衰モデル） ─────
        let decay = 1.0 - Self::clamp(dt / (self.cfg.tau_a + dt + 1e-6), 0.0, 1.0);
        self.a *= decay;
        self.v += self.a * dt;
        self.x += self.v * dt;
        self.b *= psi_b;
    ...
    pub fn reset(&mut self) {
        ...
        self.prev_a_raw = 0.0; self.a_tangential_lp = 0.0;
