...
macro_rules! lcd_display_interface {
    ($peripherals:ident, $spi:expr) => {
        shared_lcd_display_interface!($peripherals, crate::LCD_MEMORY_SIZE, $spi, $peripherals.GPIO4)
    };
}
...
macro_rules! lcd_reset_pin {
    ($peripherals:ident) => {
        OutputOpenDrain::new($peripherals.GPIO18, Level::High, Pull::Up)
    };
}
...
macro_rules! lcd_backlight_init {
    ($peripherals:ident) => {{
        let mut backlight = Output::new($peripherals.GPIO9, Level::High);
        backlight.set_high();
        Some(backlight)
    }};
}
